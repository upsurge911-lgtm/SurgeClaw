...
1. **Scalable**: Capable of running a C-Suite of specialized agents (Finance, Marketing, Dev, HR).
2. **Auditable**: Compliant with enterprise security standards like SOC 2 and GDPR.
3. **Isolated**: Preventing cross-agent data leaks and network collisions.
...
By default, running `openclaw` on a server provides a single agent environment. If you share this agent across departments:
...
graph LR
    ...
    SC -->|Vault| A[CEO Agent]
    SC -->|Vault| B[Finance Agent]
    SC -->|Vault| C[Dev Sandbox]
```

### 1. Absolute Failure Isolation (UNIX Vaults)
When you deploy OpenClaw via SurgeClaw in **Enterprise Mode**, it creates a dedicated, isolated cabinet for every agent.
- It applies **UNIX 700/600 permissions**, locking the agent's memory and configuration to the executing user.
- If the "Dev Sandbox" agent crashes or gets compromised, the "Finance Agent" remains entirely safe and online.

### 2. Built-in Compliance Ledger
Enterprise deployment requires answering the question: *"Who launched this agent, and when?"*
